...
def preprocess(image, frame_number=0):
    ...
    cv2.imwrite(f"{PREPROCESS_DIR}/denoised_{frame_number}.png",  gray)
    cv2.imwrite(f"{PREPROCESS_DIR}/binarized_{frame_number}.png",  binary)
    cv2.imwrite(f"{PREPROCESS_DIR}/cleaned_{frame_number}.png",  cleaned)
    return oriented