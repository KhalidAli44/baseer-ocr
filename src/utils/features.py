...
def normalise(char_img):
    h, w = char_img.shape
    if h == 0 or w == 0:
        return np.ones((NORM_SIZE, NORM_SIZE), dtype=np.uint8) * 255
    ...
    canvas = np.ones((NORM_SIZE, NORM_SIZE), dtype=np.uint8) * 255
...
def _outline_features(norm_img):
    ...
    if not contours:
        return np.zeros(OUTLINE_SAMPLES * 3 + OUTLINE_SAMPLES * 3, dtype=np.float32)
    ...
    def sample_contour(contour):
        total   = len(contour)
        indices = np.linspace(0, total - 1, OUTLINE_SAMPLES, dtype=int)
        sampled = contour[indices].reshape(-1, 2).astype(np.float32)
        nx      = sampled[:, 0] / (NORM_SIZE - 1)
        ny      = sampled[:, 1] / (NORM_SIZE - 1)
    ...
    inner_feat = sample_contour(inner) if inner is not None else np.zeros(OUTLINE_SAMPLES * 3, dtype=np.float32)
...
def _grid_features(norm_img):
    ...
    cell_h   = NORM_SIZE // GRID_ROWS
    cell_w   = NORM_SIZE // GRID_COLS
    features = np.zeros(GRID_ROWS * GRID_COLS, dtype=np.float32)

    for r in range(GRID_ROWS):
        for c in range(GRID_COLS):
            cell = inv[r * cell_h:(r + 1) * cell_h,
                       c * cell_w:(c + 1) * cell_w]
            features[r * GRID_COLS + c] = float(cell.mean())
...
def _direction_histogram(skel_img):
    ...
    bins      = np.linspace(-np.pi, np.pi, DIRECTION_BINS + 1)
...
def _projection_profiles(norm_img):
    ...
    h_resampled  = cv2.resize(h_profile.reshape(1, -1), (PROFILE_BINS, 1), interpolation=cv2.INTER_AREA).flatten()
    v_resampled  = cv2.resize(v_profile.reshape(1, -1), (PROFILE_BINS, 1), interpolation=cv2.INTER_AREA).flatten()
...
def _topology_features(norm_img):
    ...
    euler_norm   = float(euler.sum()) / (NORM_SIZE * NORM_SIZE * 255.0)
